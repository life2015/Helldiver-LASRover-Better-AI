...
def main():
    ...
    report=json.loads((ROOT/'build/experimental-package-report.json').read_text())
    name=report.get('zip_name','Rover-Fire-Spread-Experimental-0.1.zip')
    if not re.fullmatch(r'Rover-Fire-Spread-Experimental-[\d.]+\.zip',name):raise ValueError('Unexpected package name')
