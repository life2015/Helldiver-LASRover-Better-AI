...
def build_variant(original, show_hud):
    ...
    report.update({'installable':True,'gameplay_validated':False,'startup_tests':tests,
        ...
        'target_policy':{'attack_window_seconds':0.4,'history_limit':1,'history_seconds':1,'no_attack_seconds':1.5,'max_lock_seconds':1.5,
            ...
            'near_override':'if departing target is beyond 15m from drone and eligible alternatives exist within 15m inclusive, restrict to near pool before Recent filtering and rank by drone range',
        ...
        'hud':{'enabled':show_hud,'runtime_validated':False,'font_layout':'baseline game.dll only' if show_hud else None,'refresh_hz':10 if show_hud else 0,'placement':'top center' if show_hud else None},
    ...
    description='Laser Rover rotation: prefer enemies adjacent to the departing target; prioritize enemies within 15m of Rover when leaving a target beyond 15m. Recent: 1 record / 1 second. After 1.5s without synchronized attack state, or 1.5s on the same identified target across attack-state toggles, request nearest alternative. Normal 0.4s attack rotation takes priority. Includes Bingus Shared Loader v12 bridge. '
    ...
    out=ROOT/'releases'/package_name('v12',show_hud);out.parent.mkdir(exist_ok=True)
...
def main():
    ...
    reports=[build_variant(original,True),build_variant(original,False)]
    ...
    for builder in (build_v14,build_v15):
    ...
    (folder/'package-matrix.json').write_text(json.dumps({
        'version':VERSION,'packages':[{k:r[k] for k in ('zip_name','zip_sha256')} for r in reports]
    },indent=2),encoding='utf-8')
    print('PASS six packages: v12/v14/v15 x HUD/No HUD; shared core verified; no deployment')
