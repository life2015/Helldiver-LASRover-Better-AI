...
local function read(api,game,owner,candidates)
    ...
    for n=0,(tail-head)%128-1 do
        ...
        if u(b,24)==owner and timed(b) then chosen=b;offset=off end
    end
    if not chosen then return nil,'none' end
    ...
    local candidate
    for _,c in ipairs(candidates)do if c.id==id and c.eligible and c.identity then candidate=c;break end end
    if not candidate then return nil,'not_eligible' end
    ...
    local function valid()
        local ok,result=pcall(function()
            ...
            return b:sub(1,4)==chosen:sub(1,4) and b:sub(17,20)==chosen:sub(17,20)
                ...
                and candidate.valid() == true
        end)
        return ok and result==true
    end
    if not valid() then return nil,'changed' end
    return {id=id,identity=candidate.identity,valid=valid},'eligible'
end
function M.read(...)
    local ok,mark,status=pcall(read,...)
